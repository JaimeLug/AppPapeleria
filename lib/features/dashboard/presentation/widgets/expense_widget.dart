...
class ExpenseWidget extends ConsumerWidget {
  ...
  const ExpenseWidget({super.key, this.isDragging = false, this.onRemove, this.onResize, this.onResizeHeight});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return DashboardWidgetWrapper(
      ...
      backgroundColor: ref.watch(dashboardPaletteProvider).expense, // Full Color
      widgetId: DashboardWidgetIds.expenses,
      child: statsAsync.when(
        data: (stats) => Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Icon(Icons.money_off, size: 24, color: Colors.white),
